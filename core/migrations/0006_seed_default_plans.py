# Generated manually to auto-seed default pricing plans on migrate

from django.db import migrations

PLANS = [
    dict(code='starter', name='Starter', price_monthly=299, price_annual=2990,
         monthly_verified_bills=200, max_staff=2, tagline='Solo barber / tiny salon',
         is_target=False,
         features=['Customer QR bill verification', 'Services + prices', 'Payment recording',
                   'Digital receipts', 'Basic sales dashboard', '2 staff', '200 verified bills/mo']),
    dict(code='growth', name='Growth', price_monthly=499, price_annual=4990,
         monthly_verified_bills=500, max_staff=5, tagline='Small/medium salon', is_target=True,
         features=['Everything in Starter', '5 staff', 'Customer history', 'Staff sales',
                   'Daily reconciliation', 'Discounts/voids tracking', 'Audit history',
                   '500 verified bills/mo']),
    dict(code='business', name='Business', price_monthly=799, price_annual=7990,
         monthly_verified_bills=1000, max_staff=10, tagline='Busy salon + barber', is_target=False,
         features=['Everything in Growth', '10 staff', 'Salon + barber departments',
                   'Staff performance', 'Commission tracking', 'Revenue discrepancy alerts',
                   'Advanced reports', '1,000 verified bills/mo']),
    dict(code='pro', name='Pro', price_monthly=1299, price_annual=12990,
         monthly_verified_bills=2000, max_staff=20, tagline='Large salon/beauty centre',
         is_target=False,
         features=['Everything in Business', '20 staff', '2,000 verified bills/mo',
                   'Multiple managers', 'Advanced analytics', 'Branch support', 'Priority support']),
]


def seed_plans(apps, schema_editor):
    Plan = apps.get_model('core', 'Plan')
    for p in PLANS:
        Plan.objects.update_or_create(code=p['code'], defaults=p)


def unseed_plans(apps, schema_editor):
    Plan = apps.get_model('core', 'Plan')
    Plan.objects.filter(code__in=[p['code'] for p in PLANS]).delete()


class Migration(migrations.Migration):

    dependencies = [
        ('core', '0005_bill_payment_ref'),
    ]

    operations = [
        migrations.RunPython(seed_plans, reverse_code=unseed_plans),
    ]
